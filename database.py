...
import time
import os  # <-- CHANGE 1: Added for environment detection

# ==================== ENVIRONMENT DETECTION ====================
# Automatically handles local development AND Render.com deployment
if os.environ.get('RENDER'):
    # On Render.com, use the persistent disk
    DB_DIR = os.environ.get('RENDER_DISK_PATH', '/opt/render/project/src')
    DB_NAME = os.path.join(DB_DIR, "committee_workplace.db")
    print(f"✅ Running on Render.com. Database path: {DB_NAME}")
else:
    # Local development - keep your original behavior
    DB_NAME = "committee_workplace.db"
    print(f"✅ Running locally. Database path: {DB_NAME}")

# ==================== ENSURE DIRECTORY EXISTS ====================
# This creates the Render disk directory if it doesn't exist
...
# It ensures the database exists with dummy data, even on Render.com

def ensure_database_initialized():
    """Auto-initialize database with dummy data if empty"""
    try:
        conn = get_connection()
        cursor = conn.cursor()
        ...
        if table_exists:
            ...
            if count == 0:
                print("🔧 Database empty - seeding dummy data...")
                init_database()
                insert_dummy_data()
        ...
        print("🔧 Attempting to initialize fresh...")
        init_database()
        insert_dummy_data()
