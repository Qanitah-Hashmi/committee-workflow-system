...
from database import get_all_committees
...
<div class="main-header">
    <h1>🎯 Committee Slot Management</h1>
    <p>Manage seat allocations for committees</p>
</div>
""", unsafe_allow_html=True)
    ...
    for comm in committees:
        with st.container(border=True):
            ...
            with col1:
                ...
                st.caption(f"{comm['type']} • {comm['department']}")
            with col2:
                st.metric("Total Seats", comm['total_seats'])
            with col3:
                st.metric("Filled", comm['filled_seats'])
else:
